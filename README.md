...
This repository includes sanitized Universal Forwarder configuration files used during the lab.

| File | Purpose |
|------|---------|
| `config/inputs.conf` | Configures Windows Event Log collection (Application, System, Security). |
| `config/outputs.conf` | Example Splunk Cloud forwarding configuration with sensitive values removed. |

> **Note:** The actual `outputs.conf` generated by Splunk Cloud contains authentication material and endpoint information. The version included in this repository has been sanitized to protect credentials.
